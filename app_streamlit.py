...
st.set_page_config(
    page_title="GWAS Analysis Pipeline",
    page_icon="🧬",
    layout="wide",
...
<style>
    ...
    .gwas-header {
        background: white; border: 1px solid #E1E4EC;
    ...
    .section-title {
        ...
        padding-bottom: 8px; margin: 8px 0 12px;
        border-bottom: 1px solid #E1E4EC;
    }
    .info-box {
        background: #EEF2F8; border: 1px solid #E1E4EC;
        border-left: 3px solid #2B4C7E; border-radius: 4px;
        padding: 10px 14px; margin: 8px 0;
        font-size: .82rem; color: #565D75; line-height: 1.55;
    }
    .model-chip {
    ...
    .chip-OLS{background:#3E7BD6}.chip-MLM{background:#14B8A6}
    .chip-EMMAX{background:#DB4C8C}.chip-FarmCPU{background:#5FA624}
    .chip-GEMMA{background:#B054E0}.chip-BLINK{background:#E0972E}
...
<p>Bonferroni and suggestive thresholds · permutation/LOD threshold · PVE ·
cross-model MTA comparison</p>
</div>
""", unsafe_allow_html=True)
...
uploaded = st.sidebar.file_uploader(
    "Upload CSV / TSV / TXT", type=["csv", "tsv", "txt"]
)
text_input = st.sidebar.text_area(
    "Or paste CSV data", value=SAMPLE_DATA, height=180
)

selected_models = st.sidebar.multiselect(
    "Models to run",
    ALL_MODELS,
    default=["OLS", "MLM", "EMMAX", "FarmCPU", "GEMMA"],
)

maf_thr = st.sidebar.slider("MAF Threshold", 0.0, 0.5, 0.05, 0.01)
top_n_sl = st.sidebar.slider(
    "Fallback Top-N", 5, 50, 15, 1,
    help="Used only if the LOD threshold cannot be computed."
)
n_perm = st.sidebar.slider(
    "Permutations for LOD threshold", 20, 200, 100, 10
)
lod_thr_manual = st.sidebar.slider(
    "Manual LOD threshold", 0.0, 10.0, 0.0, 0.1,
    help="0 = automatic permutation-derived threshold."
)
pve_thr = st.sidebar.slider(
    "Minimum PVE %", 0.0, 100.0, 0.0, 1.0,
    help="0 = disabled; combined with the LOD threshold."
)

st.sidebar.markdown("### Advanced / Cofactors")
iter_max_iter = st.sidebar.slider("Max FEM/REM iterations", 2, 15, 8, 1)
farmcpu_bin_size = st.sidebar.slider(
    "FarmCPU bin size (bp)", 100_000, 5_000_000, 1_000_000, 100_000
)
blink_ld_r2 = st.sidebar.slider(
    "BLINK LD r² cutoff", 0.10, 0.99, 0.70, 0.01
)
mrmlm_screen_thresh = st.sidebar.slider(
    "mrMLM stage-1 p-value", 0.001, 0.05, 0.01, 0.001
)
mrmlm_max_candidates = st.sidebar.slider(
    "mrMLM max candidates", 5, 40, 20, 1
)
mrmlm_drop_thresh = st.sidebar.slider(
    "mrMLM stage-2 drop threshold", 0.01, 0.20, 0.05, 0.01
)
mlm_n_pca = st.sidebar.slider("MLM kinship PCs", 0, 10, 3, 1)
gemma_n_pca = st.sidebar.slider("GEMMA kinship PCs", 0, 10, 0, 1)
mrmlm_n_pca = st.sidebar.slider("mrMLM kinship PCs", 0, 10, 3, 1)
fastmrmlm_n_pca = st.sidebar.slider("FASTmrMLM kinship PCs", 0, 10, 3, 1)

# --------------------------- Plot configuration ------------------------------
with st.expander("Plot Customization", expanded=False):
    p1, p2, p3 = st.tabs([
        "Manhattan / QQ", "Circos / LD / PCA", "Kinship / Effect / Dashboard"
    ])

    with p1:
        c1, c2, c3 = st.columns(3)
        with c1:
            mh_w = st.slider("Manhattan width", 8, 24, 16, 1)
            mh_h = st.slider("Manhattan height", 3.0, 12.0, 5.0, 0.5)
            mh_fs = st.slider("Manhattan font", 6, 18, 9, 1)
            mh_fc = st.color_picker("Manhattan font color", "#2D3142")
        with c2:
            mh_ms = st.slider("Manhattan marker size", 2, 60, 12, 1)
            mh_alpha = st.slider("Manhattan marker alpha", 0.1, 1.0, 0.75, 0.05)
            mh_bg = st.color_picker("Manhattan background", "#FFFFFF")
            mh_panel = st.color_picker("Manhattan panel", "#FAFBFF")
        with c3:
            mh_sig = st.color_picker("Bonferroni line", "#E15759")
            mh_sug = st.color_picker("Suggestive line", "#4E79A7")
            mh_dot = st.color_picker("Manhattan dot color", "#4E79A7")
            mh_show_logp = st.checkbox("Show -log₁₀(p) right axis", True)

        st.markdown("#### QQ")
        c1, c2, c3 = st.columns(3)
        with c1:
            qq_w = st.slider("QQ width", 5.0, 16.0, 7.0, 0.5)
            qq_h = st.slider("QQ height", 5.0, 16.0, 7.0, 0.5)
            qq_fs = st.slider("QQ font", 6, 18, 10, 1)
            qq_fc = st.color_picker("QQ font color", "#2D3142")
        with c2:
            qq_ms = st.slider("QQ marker size", 5, 80, 22, 1)
            qq_dot = st.color_picker("QQ dot color", "#A78BFA")
            qq_line = st.color_picker("QQ regression line", "#E15759")
        with c3:
            qq_fill = st.color_picker("QQ CI fill", "#A78BFA")
            qq_bg = st.color_picker("QQ background", "#FFFFFF")
            qq_panel = st.color_picker("QQ panel", "#FAFBFF")

    with p2:
        c1, c2, c3 = st.columns(3)
        with c1:
            nc_models = st.multiselect(
                "Circos models", ALL_MODELS,
                default=["OLS", "MLM", "EMMAX", "FarmCPU", "GEMMA"]
            )
            nc_theme = st.radio("Circos theme", ["Bright", "Dark"], index=0)
        with c2:
            nc_rings_per_panel = st.slider("Rings per panel", 2, 4, 3, 1)
            nc_w = st.slider("Circos width", 10, 30, 20, 1)
            nc_h = st.slider("Circos height", 10, 30, 20, 1)
        with c3:
            nc_fs = st.slider("Circos font", 6, 22, 11, 1)

        st.markdown("#### LD Decay")
        c1, c2, c3 = st.columns(3)
        with c1:
            ld_w = st.slider("LD width", 8, 24, 16, 1)
            ld_h = st.slider("LD height", 3.0, 12.0, 6.0, 0.5)
        with c2:
            ld_fs = st.slider("LD font", 6, 18, 9, 1)
            ld_dot = st.color_picker("LD dot", "#4E79A7")
        with c3:
            ld_line = st.color_picker("LD trend line", "#E15759")

        st.markdown("#### PCA")
        c1, c2 = st.columns(2)
        with c1:
            pc_w = st.slider("PCA width", 8, 22, 14, 1)
            pc_h = st.slider("PCA height", 4, 14, 10, 1)
        with c2:
            pc_fs = st.slider("PCA font", 6, 18, 9, 1)

    with p3:
        st.markdown("#### Kinship / Dendrogram")
        c1, c2, c3 = st.columns(3)
        with c1:
            ki_w = st.slider("Kinship width", 8, 22, 14, 1)
            ki_h = st.slider("Kinship height", 6, 20, 12, 1)
            ki_fs = st.slider("Kinship font", 6, 18, 9, 1)
        with c2:
            ki_dot = st.color_picker("Kinship dot/bar", "#4E79A7")
            ki_bg = st.color_picker("Kinship background", "#FFFFFF")
        with c3:
            ki_panel = st.color_picker("Kinship panel", "#FAFBFF")

        st.markdown("#### Effect Sizes")
        c1, c2 = st.columns(2)
        with c1:
            ef_w = st.slider("Effect width", 8, 22, 14, 1)
            ef_h = st.slider("Effect height", 5, 18, 10, 1)
            ef_fs = st.slider("Effect font", 6, 18, 9, 1)
        with c2:
            ef_pos = st.color_picker("Positive effect", "#4E79A7")
            ef_neg = st.color_picker("Negative effect", "#E15759")
            ef_fill = st.color_picker("Distribution fill", "#A78BFA")
            ef_bg = st.color_picker("Effect background", "#FFFFFF")

        st.markdown("#### All-Models Manhattan / Dashboard")
        c1, c2, c3 = st.columns(3)
        with c1:
            co_w = st.slider("All-model MH width", 8, 24, 16, 1)
            co_h = st.slider("All-model MH height/model", 2.0, 8.0, 3.5, 0.5)
        with c2:
            co_sig = st.color_picker("All-model Bonferroni", "#E15759")
            co_sug = st.color_picker("All-model suggestive", "#4E79A7")
        with c3:
            db_w = st.slider("Dashboard width", 8, 24, 15, 1)
            db_h = st.slider("Dashboard height", 5, 18, 10, 1)
            db_dot = st.color_picker("Dashboard bar/dot", "#4E79A7")

# ------------------------------- Run pipeline --------------------------------
st.markdown('<div class="section-title">Analysis</div>', unsafe_allow_html=True)

run = st.button("Run Complete GWAS Pipeline", type="primary", use_container_width=True)
...
if run:
    ...
    args = [
        file_obj, text_input, selected_models, maf_thr, top_n_sl, n_perm,
        lod_thr_manual, pve_thr,
        iter_max_iter, farmcpu_bin_size, blink_ld_r2,
        mrmlm_screen_thresh, mrmlm_max_candidates, mrmlm_drop_thresh,
        mlm_n_pca, mrmlm_n_pca, fastmrmlm_n_pca, gemma_n_pca,
    ...
    with st.spinner("Running GWAS pipeline — this may take time for large SNP datasets..."):
        try:
            result = run_gwas(*args)
        except Exception as e:
            st.error(f"GWAS pipeline failed: {e}")
            result = None

    if result is not None:
        # run_gwas returns:
        # 11 images + Excel + ZIP + TXT + summary + cache_state
        if len(result) >= 16 and isinstance(result[-1], dict):
            st.session_state["gwas_results"] = result[:11]
            st.session_state["gwas_excel"] = result[11]
            st.session_state["gwas_zip"] = result[12]
            st.session_state["gwas_summary_file"] = result[13]
            st.session_state["gwas_summary"] = result[14]
            st.session_state["gwas_state"] = result[15]
            st.success("GWAS analysis completed.")
        else:
            # Preserve the original error convention as much as possible.
            msg = result[-1] if result and isinstance(result[-1], str) else "Unknown error"
            st.error(msg)

# -------------------------------- Results ------------------------------------
results = st.session_state.get("gwas_results")

if results:
    st.markdown('<div class="section-title">Results</div>', unsafe_allow_html=True)
    result_tabs = st.tabs([
        "Manhattan", "QQ Primary", "QQ All Models", "QQ Overlay", "LD Decay",
        "PCA", "Kinship", "Effect Sizes", "All Models MH", "Dashboard",
        "Circos", "Summary", "Downloads"
    ])

    labels = [
        "Manhattan Plot — Primary Model",
        "QQ Plot — Primary Model",
        "QQ Plots — All Models",
        "QQ Overlay",
        "LD Decay",
        "PCA",
        "Kinship & Dendrogram",
        "Effect Sizes & Volcano Plot",
        "All-Models Manhattan",
        "Summary Dashboard",
        "Circos — Density Ring + Multi-Model Rings",
    ]

    for tab, image, label in zip(result_tabs[:11], results, labels):
        with tab:
            if image is not None:
                st.image(image, caption=label, use_container_width=True)
            else:
                st.warning(f"{label} was not generated.")

    with result_tabs[11]:
        st.code(st.session_state.get("gwas_summary", ""), language="text")

    with result_tabs[12]:
        excel_path = st.session_state.get("gwas_excel")
        zip_path = st.session_state.get("gwas_zip")
        summary_path = st.session_state.get("gwas_summary_file")
        ...
        if excel_path and os.path.exists(excel_path):
            with open(excel_path, "rb") as f:
                st.download_button(
                    ...
                    f.read(),
                    file_name="gwas_results_top15.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
        if zip_path and os.path.exists(zip_path):
            with open(zip_path, "rb") as f:
                st.download_button(
                    "Download All Plots (PNG ZIP)",
                    f.read(),
                    file_name="gwas_plots.zip",
        ...
        if summary_path and os.path.exists(summary_path):
            with open(summary_path, "rb") as f:
                st.download_button(
                    ...
                    f.read(),
                    file_name="gwas_summary.txt",
                    mime="text/plain",
                )

    # -------------------------- Live Manhattan redraw ------------------------
    st.markdown('<div class="section-title">Manhattan Preview</div>', unsafe_allow_html=True)
    st.caption(
        "Changing the controls above causes a Streamlit rerun. "
        "Use the button below to redraw the Manhattan plot from cached GWAS results "
        "without fitting the GWAS models again."
    )
    if st.button("Refresh Manhattan from Cached Results"):
        state = st.session_state.get("gwas_state")
        if state:
            try:
                live_img = refresh_manhattan_live(
                    state, mh_w, mh_h, mh_fs, mh_fc, mh_ms, mh_alpha,
                    mh_bg, mh_sig, mh_sug, mh_dot, mh_panel, mh_show_logp
                )
                if live_img is not None:
                    st.image(live_img, caption="Updated Manhattan Plot", use_container_width=True)
            except Exception as e:
                st.error(f"Could not redraw Manhattan plot: {e}")

# ----------------------------- Compare models --------------------------------
with st.expander("Compare Models", expanded=False):
    st.info(
        "Run the pipeline first. This comparison reuses cached model results "
        "and does not refit the GWAS models."
    )
    ...
    cmp_mode = st.radio(
        ...
        ["LOD (permutation) — recommended", "Bonferroni (0.05/m)", "Top-N rank"],
        key="cmp_mode",
    )
    cmp_top_n = st.slider(
        "N for Top-N/fallback", 5, 50, 15, 1, key="cmp_top_n"
    )
    c1, c2, c3 = st.columns(3)
    with c1:
        cmp_w = st.slider("Comparison width", 10, 24, 16, 1)
    with c2:
        cmp_h = st.slider("Comparison height", 6, 16, 10, 1)
    with c3:
        cmp_fs = st.slider("Comparison font", 6, 16, 10, 1)

    if st.button("Find Common MTAs", type="secondary"):
        if not state:
            st.warning("Run the GWAS pipeline first.")
        elif not cmp_models:
            st.warning("Select at least one model.")
        else:
            try:
                img, table, fig_path, table_path = run_compare_models(
                    state, cmp_models, cmp_mode, cmp_top_n,
                    cmp_w, cmp_h, cmp_fs
                )
                if img is not None:
                    st.image(img, caption="Common-MTA Comparison", use_container_width=True)
                st.dataframe(table, use_container_width=True)

                if fig_path and os.path.exists(fig_path):
                    with open(fig_path, "rb") as f:
                        st.download_button(
                            "Download Common-MTA Figure",
                            f.read(),
                            file_name="common_MTA_figure.png",
                            mime="image/png",
                        )
                if table_path and os.path.exists(table_path):
                    with open(table_path, "rb") as f:
                        st.download_button(
                            "Download Common-MTA Table",
                            f.read(),
                            file_name="common_MTA_table.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        )
            except Exception as e:
...
st.caption(
    "GWAS computational engine preserved from the original application; "
    "only the Gradio interface/deployment layer has been replaced with Streamlit."
)
